...
local function _get_function_names(module_name, fully_qualified)
  local l = vim.fn.getcompletion(('lua %s'):format(module_name), 'cmdline')
  local filtered = vim.iter(l)
    :filter(function(s)
      local is_func = assert(loadstring(('return type(%s%s) == "function"'):format(module_name, s)))()
      return is_func
    end)
    :map(function(s)
      return ('%s%s'):format(fully_qualified and module_name or '', s)
    end)
    :totable()
  return filtered
end
--- Gets child module names of the given module.
--- @param m string Module name.
--- @return string[] # Child modules of the given module.
local function _get_module_names(m)
  local l = vim.fn.getcompletion((('lua %s.'):format(m)), 'cmdline')
  local deprecated = {
    ['vim.F'] = true,
    ['vim.opt'] = true,
    ['vim.opt_global'] = true,
    ['vim.opt_local'] = true,
    ['vim.loop'] = true,
    ['vim.uv'] = true,  -- Not actually deprecated, but its names come from a third party.
    ['vim.lpeg'] = true,  -- Not actually deprecated, but its names come from a third party.
  }
  local filtered = vim.iter(l)
    :map(function(s) return ('%s.%s'):format(m, s) end)
    :filter(function(module_name)
      local is_table = assert(loadstring(('return type(%s) == "table"'):format(module_name)))()
      return not deprecated[module_name] and is_table
    end)
    :totable()
  return filtered
end
local function _list_all_functions(fully_qualified)
  vim.cmd([[
    new
  ...
  for _, m in ipairs(_get_module_names('vim')) do
    vim.api.nvim_put(_get_function_names(m .. '.', true), 'l', true, fully_qualified)
  end
